...
function gas(method, ...args) {
  return new Promise((resolve, reject) => {
    ...
    const timeoutByMethod = {
      ...
      getDashboardData: 60000,
      createWebsite: 120000,
      uploadFileBase64: 60000
    };
    ...
    const timer = setTimeout(() => {
      ...
      if (method === 'loginAdmin') {
        hint = ' Periksa APPS_SCRIPT_URL, deployment Web App, dan FRONTEND_ORIGIN.';
      } else if (method === 'createWebsite') {
        hint = ' Repository mungkin sedang diproses; muat ulang dashboard sebelum mencoba lagi.';
      }
...
function updateCreateTemplateState() {
  ...
  const appScriptTemplate = template === 'appscript';
  const appUrlField = document.getElementById('appScriptUrlField');
  const appUrlInput = document.getElementById('appScriptUrlInput');

  notice.hidden = !noTemplate;
  appUrlField.hidden = !appScriptTemplate;
  appUrlInput.required = appScriptTemplate;
  ...
  } else if (appScriptTemplate) {
    templateHint.textContent = 'Membuat halaman iframe fullscreen untuk Google Apps Script.';
    pages.disabled = false;
    pagesHint.textContent = 'GitHub Pages akan menampilkan Web App Apps Script dalam iframe.';
  } else {
...
async function loadTemplate(name, title, extraVars={}){
  if(name==='none') return [];
  ...
  const vars={
    '{{TITLE}}':title||'Website Baru',
    '{{APP_URL}}':extraVars.appUrl||''
  };
...
async function createWebsite(e){
  ...
  try{
    ...
    if(template==='appscript'){
      if(!/^https:\/\/script\.google\.com\//i.test(appUrl)){
        throw new Error('URL Apps Script harus menggunakan https://script.google.com/.');
      }
      if(!/\/exec(?:[?#].*)?$/i.test(appUrl)){
        throw new Error('Gunakan URL deployment Apps Script Web App yang berakhir dengan /exec.');
      }
    }

    const files=await loadTemplate(template,title,{appUrl});
...
async function testConnection(){
  ...
  try{
    const r=await gas('testGithubConnection');
    const pill=document.getElementById('connectionPill');
    pill.textContent='Terhubung';
    pill.className='status good';
    document.getElementById('tokenStatus').textContent='Sudah diatur';
    document.getElementById('passwordStatus').textContent='Sudah diatur';
    document.getElementById('backendStatus').textContent='Apps Script';
    toast(`Terhubung sebagai @${r.login}.`,'success');
  }catch(err){
    const pill=document.getElementById('connectionPill');
...
function switchView(v){
  const labels={dashboard:['Dashboard','Kelola repository dan GitHub Pages.'],repositories:['Repository','Lihat dan kelola repository GitHub.'],files:['File Manager','Edit, buat, hapus, dan upload file.'],settings:['Pengaturan','Status koneksi dan arsitektur aplikasi.']};
  const target=document.getElementById('view-'+v);
  if(!target||!labels[v]){toast('Menu tidak ditemukan.','error');return;}
  state.currentView=v;
  document.querySelectorAll('.view').forEach(x=>x.classList.remove('active'));
  target.classList.add('active');
  document.querySelectorAll('.nav-item[data-view]').forEach(x=>x.classList.toggle('active',x.dataset.view===v));
  document.getElementById('pageTitle').textContent=labels[v][0];
  document.getElementById('pageSubtitle').textContent=labels[v][1];
  document.getElementById('sidebar').classList.remove('open');
}
